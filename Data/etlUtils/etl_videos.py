...
import Data.etlUtils.etl_shared
...
def main(json_path="Data/videos.json", collection=None, db=None, demoMode=False):
    ...
    if demoMode:
        video_infos = Data.etlUtils.etl_shared.subsampleForDemo(video_infos, Data.etlUtils.etl_shared.DEMO_VIDEO_SAMPLES)

    # Extract subtitles and chapters
    # all_papers_texts = map(extract_pdf, paper_data)
    all_subtitles = []

    USE_PARALLEL = Data.etlUtils.etl_shared.PARALLEL_USE
    ...
    def write_chunks_to_database(ending_index: int, chunk_index: int):
        ...
        documents = Data.etlUtils.etl_shared.unchunk(all_subtitles)

        num_chunks = 10
        chunked_documents = Data.etlUtils.etl_shared.chunk_into(documents, num_chunks)
        res = []
        jsonlines = []
        for myvideodoc in tqdm(chunked_documents, total=(num_chunks),
                               desc="Sending chuncks of videos to mongdb and local json"):
            r = Data.etlUtils.etl_shared.add_to_mongo_db(myvideodoc)
            res.append(r)

            res_json = add_video_to_local_db(myvideodoc)
            jsonlines.append(res_json)

        jsonlines = Data.etlUtils.etl_shared.unchunk(jsonlines)
    ...
    if USE_PARALLEL:
        ...
        n_processes = Data.etlUtils.etl_shared.PARALLEL_NUM_PROCESSES
        with Pool(processes=n_processes) as pool:
            results: SubsExtractResults = Data.etlUtils.etl_shared.imap_unordered_bar(extract_subtitles, video_infos, n_processes=n_processes, desc="Parallel extraction of subtitles: ")
...
def create_documents(chapters, id, video_title):
    ...
    documents = Data.etlUtils.etl_shared.add_metadata(documents)
