...
def main(json_path, collection=None, db=None):
    ...
    for contentName, content in markdown_corpus.items():
        ...
        documents = (
                Data.etlUtils.etl_shared.unchunk(  # each lecture creates multiple documents, so we flatten
                map(functools.partial(to_documents), content))
            )

        chunked_documents = Data.etlUtils.etl_shared.chunk_into(documents, 10)
        res = list(map(Data.etlUtils.etl_shared.add_to_mongo_db, chunked_documents))
        res = res
