...
image = Data.etlUtils.etl_shared.image.pip_install(
    "arxiv==1.4.7",
    "pypdf==3.8.1",
)
...
def main(json_path="Data/pdfpapers.json", collection=None, db=None):
    ...
    documents = Data.etlUtils.etl_shared.unchunk(extract_pdf.map(paper_data, return_exceptions=True))

    with Data.etlUtils.etl_shared.stub.run():
        chunked_documents = Data.etlUtils.etl_shared.chunk_into(documents, 10)
        list(
            Data.etlUtils.etl_shared.add_to_mongo_db.map(
