...
def transform_papers_to_json(papers_path, demoMode=False):
    ...
    if demoMode:
        pdf_infos = Data.etlUtils.etl_shared.subsampleForDemo(pdf_infos, Data.etlUtils.etl_shared.DEMO_PDF_SAMPLES)
    ...
    if Data.etlUtils.etl_shared.PARALLEL_USE:
        from multiprocessing import Pool
        n_processes = Data.etlUtils.etl_shared.PARALLEL_NUM_PROCESSES
        with Pool(processes=n_processes) as pool:
            results = Data.etlUtils.etl_shared.imap_unordered_bar(get_pdf_url, paper_data,
        ...
        with Pool(processes=n_processes) as pool:
            results = Data.etlUtils.etl_shared.imap_unordered_bar(extract_pdf, paper_data,
                                                                  n_processes=n_processes,
    ...
    documents = Data.etlUtils.etl_shared.unchunk(all_papers_texts)
    ...
    n_chunks = 10
    chunked_documents = Data.etlUtils.etl_shared.chunk_into(documents, n_chunks)
    ...
    for mydoc in tqdm(chunked_documents, total=n_chunks, desc="Writing chunks to mongodb and setting up local pdf database"):
        res = Data.etlUtils.etl_shared.add_to_mongo_db(mydoc)
