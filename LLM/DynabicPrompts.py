...
def set_templates():
    ...
    if USE_CASE == UseCase.Default or USE_CASE == UseCase.SmartHome:
        ...
        template_securityOfficer_instruction_rag_nosources_funccalls_devicesByIPLogs = """\
                Write only the following string and no other words, do not start your response with Sure.
                "Ok, I will show you the pandas dataset according to your request. I will invoke FUNC_CALL dynabicagenttools.show_outlier_ips_usage Params '../RAGSupport/dataForRAG//Hospital_DDoSSnapshot_Data/DATASET_LOGS_HACKED_False.csv' '../RAGSupport/dataForRAG/Hospital_DDoSSnapshot_Data/DATASET_LOGS_HACKED_True.csv'"
                {context}"""
        ...
        template_securityOfficer_instruction_rag_nosources_funccalls_comparisonMapRequests = """\
                Write only the following string and no other words, do not start your response with Sure.
                "I will invoke FUNC_CALL dynabicagenttools.shopComparativeColumnsDatasets Params '../RAGSupport/dataForRAG/Hospital_DDoSSnapshot_Data/DATASET_LOGS_HACKED_True.csv' '../RAGSupport/dataForRAG/Hospital_DDoSSnapshot_Data/DATASET_LOGS_HACKED_False.csv'"
                {context}"""

        template_securityOfficer_instruction_rag_nosources_funccalls_firewallInsert = """\
                Write only the following string and no other words, do not start your response with Sure. Do not write like I provided you the code.
                '''
                df1 = pd.read_csv("../RAGSupport/dataForRAG/Hospital_DDoSSnapshot_Data/FIREWALL_PROCESSES.csv")

                new_row = ('IP': {param_ip}, 'NAME': {param_name}, 'DATE': datetime.now(), 'BLOCKED':1)

                df1=pd.concat([df1, pd.DataFrame([new_row])], ignore_index=True)

                df1.to_csv("../RAGSupport/dataForRAG/Hospital_DDoSSnapshot_Data/FIREWALL_PROCESSES.csv")
                '''
                {context}"""
