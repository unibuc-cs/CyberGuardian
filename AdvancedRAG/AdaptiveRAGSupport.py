...
class AdaptiveRAGSupport:
    ...
    def __init__(self, model: Union[HuggingFacePipeline, None],
        ...
        if all(env_model_things):
            ...
            assert any(env_model_things) is False, \
                "Either all or none of the model, tokenizer and retriever should be provided"
    ...
    def get_docs_content_by_query(self, query: str, num_max_docs_returned: int,
                                  format_as_single_text: bool, verbose: bool = False) -> str:
        """
        Args:
            query: the query
        ...
        if format_as_single_text is True:
            docs_content = advancedRagUtils.format_docs_to_str(docs_list)

        return docs_content

    # Answer a question with RAG
    def answer_with_rag(self, /, question: str, docs_content: Union[str, List[Document]], skip_prompt=True) -> str:
        """
        Args:
        ...
        Returns:
            Str
        """
        if List[Document] == type(docs_content):
            docs_content = advancedRagUtils.format_docs_to_str(docs_content)
        ...
        assert type(res) is str, f"Expected a string but got {type(res)}"
        return res
    ...
    def answer_grader(self, question: str, answer: str, skip_prompt=True) -> str:
        ...
        Returns:
            Str
        """
        answer_grader_chain = self.prompts_factory.answer_grader_prompt | self.llm_pipeline.bind(
            skip_prompt=skip_prompt) | JsonOutputParser()
        return answer_grader_chain.invoke({"question": question, "generation": answer})

    # Checks if the answer is hallucinated or sustained by the documents. JSON output by the hallucination grader
    def hallucination_grader(self, answer: str, docs_content: Union[str, List[Document]], skip_prompt=True) -> str:
        """
        Args:
            answer: the answer
            docs_content: the documents content as context
        ...
        if type(docs_content) is not str:
            docs_content = advancedRagUtils.format_docs_to_str(docs_content)

        hallucination_grader_chain = self.prompts_factory.hallucination_grader_prompt | self.llm_pipeline.bind(
            skip_prompt=skip_prompt) | JsonOutputParser()
        res = hallucination_grader_chain.invoke({"documents": docs_content, "generation": answer})
        return res
    ...
    def question_router(self, question: str, skip_prompt=True) -> Dict[str, Any]:
        ...
        question_router_chain = self.prompts_factory.question_router_prompt | self.llm_pipeline.bind(
            skip_prompt=skip_prompt)# | JsonOutputParser()
        res = question_router_chain.invoke({"question": question,
                                            "expert_domains_text":
                                            "llm agents, agent memory, prompt engineering, adversarial attacks"})
        return res
